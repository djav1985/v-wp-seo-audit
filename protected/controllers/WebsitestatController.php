...
class WebsitestatController extends Controller {
	...
	public function actionGenerateHTML( $domain ) {
		...
		$this->title = 'Website review | ' . ->website['idn'];
		$description = 'Website review | ' . ->website['idn'];
		$cs          = Yii::app()->clientScript;
		$cs->registerMetaTag( 'Website review | ' . ->website['idn'], 'keywords' );
	...
	protected function createPdfFromHtml( $html, $pdfFile, $filename ) {
		...
		$pdf->SetTitle( 'Website review ' . ->website['idn'] );
		$pdf->SetSubject( 'Website review ' . ->website['idn'] );
