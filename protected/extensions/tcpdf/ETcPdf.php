...
class ETcPdf
{
   ...
   public function __construct($orientation, $unit, $format, $unicode, $encoding)
   {
      if ($orientation != 'P' && $orientation != 'L')
         throw new CException('The orientation must be "P" or "L"');

      if (!in_array($unit, array('pt', 'mm', 'cm', 'in')))
         throw new CException('The unit must be "pt", "in", "cm" or "mm"');

      if (!is_string($format) && !is_array($format))
         throw new CException('The format must be string or array.');
      if (is_string($format)) {
         if (!in_array($format, array('A3', 'A4', 'A5', 'Letter', 'Legal')))
            throw new CException('The format must be one of A3, A4, A5, Letter or Legal');
      }
      else {
         if (!is_numeric($format[0]) && !is_numeric($format[1]))
            throw new CException('The format must be array(w, h)');
      }

      if (!is_bool($unicode))
         throw new CException('"unicode" must be a boolean value');
